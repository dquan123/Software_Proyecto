...
export default function AdminAssignments() {
  ...
  return <AdminLayout><AdminPageHeader title="Asignaciones" description="Asigna solicitudes pendientes a los asesores disponibles." />{notice && <p className="admin-feedback" role="status">{notice}</p>}<AdminResourceState {...resource} isEmpty={!cases.length && !advisors.length} empty="No hay solicitudes ni asesores disponibles." />
    {!resource.isLoading && !resource.error && <div className="admin-assignment-grid"><section className="admin-list-card"><header className="admin-card-title"><h3>Casos sin asignar <span>{cases.length}</span></h3></header>{cases.length ? <div className="admin-case-list">{cases.map((item) => <button type="button" className={selected?.id === item.id ? "is-selected" : ""} key={item.id} onClick={() => setSelected(item)}><strong>{item.nombre}</strong><span>{item.perfil || "Sin perfil"}</span><small>{item.etapa_actual}</small></button>)}</div> : <p className="admin-inline-empty">No hay casos sin asignar.</p>}</section><section className="admin-list-card"><header className="admin-card-title"><h3>Asesores disponibles</h3><AdminSearch value={query} onChange={setQuery} placeholder="Buscar asesor..." /></header><div className="admin-case-list">{filteredAdvisors.map((advisor) => { const load = Math.round(advisor.asignados / advisor.capacidad * 100); const disabled = !selected || !advisor.disponible || load >= 100; return <article className="admin-assignee" key={advisor.id}><b>{advisor.nombre.slice(0,2).toUpperCase()}</b><div><strong>{advisor.nombre}</strong><span>{advisor.asignados} / {advisor.capacidad} casos · {load}%</span><span className="admin-load-track"><i style={{ width: `${Math.min(100, load)}%` }} /></span></div><button type="button" disabled={disabled} onClick={() => assign(advisor)}>{selected ? "Asignar" : "Selecciona un caso"}</button></article>; })}</div></section></div>}
  </AdminLayout>;
}
