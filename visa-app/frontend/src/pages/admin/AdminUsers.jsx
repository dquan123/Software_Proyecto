...
export default function AdminUsers() {
  ...
  return <AdminLayout>
    ...
    <section className="admin-list-card">
      ...
      {!resource.isLoading && !resource.error && users.length > 0 && <><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Usuario</th><th>Rol / tipo</th><th>Estado</th><th>Actividad / asesor</th><th>Acción</th></tr></thead><tbody>{visible.length ? visible.map((user) => <tr key={user.id}><td><span className="admin-user-cell"><b>{user.nombre.slice(0,2).toUpperCase()}</b><span><strong>{user.nombre}</strong><small>{user.correo}</small></span></span></td><td><strong>{user.rol === "cliente" ? "Solicitante" : user.rol === "asesor" ? "Asesor" : "Administrador"}</strong><small>{user.perfil || "N/A"}</small></td><td><span className={`admin-status admin-status--${user.activo ? "approved" : "correction"}`}>{user.activo ? "Activo" : "Inactivo"}</span></td><td><small>{user.rol === "asesor" ? `${user.asignados} casos asignados` : user.actividad ? new Date(user.actividad).toLocaleDateString("es-GT") : "Sin actividad"}</small>{user.rol === "cliente" && <strong>{user.asesor || "Sin asignar"}</strong>}</td><td><Link className="admin-secondary-button" to={`/admin/users/${user.id}`}>Ver detalle</Link><button className="admin-secondary-button" type="button" onClick={() => toggleUser(user)}>{user.activo ? "Desactivar" : "Activar"}</button></td></tr>) : <tr><td colSpan="5" className="admin-table-state">No hay usuarios que coincidan con los filtros.</td></tr>}</tbody></table></div><AdminPagination page={page} pages={pages} onChange={setPage} total={filtered.length} visible={visible.length} /></>}
    </section>
